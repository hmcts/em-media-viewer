...
export const getBookmarkInfo = createSelector(
  ...
  (bookmarkNodes, docId, pdfPosition, pages) => {
    return {
      ...
      yCoordinate: (pages[pdfPosition.pageNumber].styles.height / parseFloat(pages[pdfPosition.pageNumber].scaleRotation.scale)) - (pdfPosition.top * (pages[pdfPosition.pageNumber].viewportScale / parseFloat(pages[pdfPosition.pageNumber].scaleRotation.scale))),
